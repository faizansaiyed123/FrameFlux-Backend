...
class ProcessingJob(Base):
    ...
    media_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("media.id", ondelete="CASCADE"), nullable=True, index=True)
    media_version_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("media_versions.id", ondelete="CASCADE"), nullable=True, index=True)
    ...
    operation_params: Mapped[str | None] = mapped_column(Text, nullable=True)
    retry_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    max_retries: Mapped[int] = mapped_column(Integer, default=3, nullable=False)
    arq_job_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
